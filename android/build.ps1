...
# 3b. link manifest + resources into base apk
& $aapt2 link -o "$out\base.apk" -I $androidJar "$out\res.flata" --manifest "$app\AndroidManifest.xml" --min-sdk-version 17 --target-sdk-version 22
...
& $apksigner sign --ks "$root\camoverlay-release.keystore" --ks-pass pass:camoverlay --ks-key-alias camoverlay --key-pass pass:camoverlay --min-sdk-version 17 --out "$out\camoverlay.apk" "$out\aligned.apk"
